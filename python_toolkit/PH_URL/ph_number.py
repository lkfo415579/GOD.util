    ...
    def recover(self,text,ori_line):
        ...
        if list_tags:
            ...
            for match in list_tags:
                try:
                    if match[1] == "EMAIL":
                        text = text.replace(match[0],email_list[int(match[2])-1])
                    elif match[1] == "URL":
                        text = text.replace(match[0],url_list[int(match[2])-1][0])
                except:
                    #print "FUCKED"
                    pass
...
if __name__ == '__main__':
    ...
    target_file = codecs.open(sys.argv[2],"rb")
    source_output = codecs.open(sys.argv[1][:-3]+".PH"+sys.argv[1][-3:],"wb")
    target_output = codecs.open(sys.argv[2][:-3]+".PH"+sys.argv[2][-3:],"wb")
    for line in source_file:
        target_line = target_file.readline().strip()
        #sys.stdout.write(Real_Generizer.tokenize(line).strip()+"\n")
        s_output_line,t_output_line = Real_Generizer.tokenize_two_line(line,target_line)
