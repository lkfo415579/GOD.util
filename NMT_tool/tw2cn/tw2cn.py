...
class Server( tornado.web.RequestHandler):
    ...
    def tw2cn( self, task) :
        ...
        #print zhconv
        if task['srcl'][-2:] == 'tw':
            #output_sent = HanziConv.toSimplified(sentence)
            output_sent = zhconv.convert(sentence, 'zh-cn')
        else:
            output_sent = zhconv.convert(sentence, 'zh-tw')
            #output_sent = traditionalize(sentence)
            #output_sent = HanziConv.toTraditional(sentence)
